...
class FinetuneModel:
                ...
                try:
                    ...
                    with autocast(dtype=torch.float16):
                        if self.model_name in ["sdxl", "kandinsky"]:
                            ...
                            target_values = noise
                            processed_prompts = [summarize_prompt(p, max_tokens=77) for p in prompts]
                            inputs = self.tokenizer(
                                processed_prompts, padding="max_length", max_length=77, truncation=True, return_tensors="pt"
                            ).to(self.device)
    ...
    def generate_image(self, prompt_template, topic_description, theory="cbt"):
        ...
        try:
            ...
            if self.model_name == "deepfloyd_if":
                prompt = summarize_prompt(prompt, max_tokens=77)
            if self.unet:
                self.unet.eval()
            if self.text_encoder:
                self.text_encoder.eval()
    ...
    def fine_tune(self, dataset_path, epochs=1, batch_size=1, learning_rate=1e-5, val_split=0.2, gradient_accumulation_steps=4):
