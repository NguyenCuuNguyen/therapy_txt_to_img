...
def summarize_prompt(prompt, max_tokens=64):
...
class FinetuneModel:
    ...
    def load_model(self):
        ...
        try:
            ...
            if self.model_name == "sdxl":
                ...
                self.unet = pipe.unet
                self.scheduler = pipe.scheduler
                self.text_encoder.to(dtype=self.dtype, device=self.device)
                self.unet.to(dtype=self.dtype, device=self.device)
                ...
                try:
                    ...
                    if isinstance(self.text_encoder, T5EncoderModel):
                        ...
                        self.logger.info("Enabled gradient checkpointing for DeepFloyd IF T5 text encoder.")
                    elif hasattr(self.text_encoder, 'gradient_checkpointing_enable'):
                        self.text_encoder.gradient_checkpointing_enable()
                        self.logger.info("Enabled gradient checkpointing for DeepFloyd IF text encoder.")
                    else:
                        self.logger.warning("Text encoder does not support gradient checkpointing.")
                    self.logger.info("Enabled gradient checkpointing for DeepFloyd IF UNet.")
                    try:
                        from xformers.ops import MemoryEfficientAttentionFlashAttentionOp
                        self.unet.enable_xformers_memory_efficient_attention()
                        self.text_encoder.enable_xformers_memory_efficient_attention()
                        self.logger.info("Enabled xFormers memory-efficient attention for UNet and text encoder.")
                    except ImportError:
                        self.logger.info("xFormers not installed, skipping memory-efficient attention.")
                except Exception as e:
                    self.logger.warning(f"Could not enable gradient checkpointing or xFormers: {e}")
            else:
    ...
    def modify_architecture(self, apply_lora_to_text_encoder=True, apply_lora_to_unet=True):
        ...
        self.logger.info(f"Configuring LoRA for {self.model_name} (TextEncoder: {apply_lora_to_text_encoder}, UNet/Prior/Decoder: {apply_lora_to_unet})")
        lora_r = 8
        lora_alpha = 16
        lora_dropout = 0.1
        lora_bias = "none"
            ...
            if apply_lora_to_unet and self.unet:
                ...
                if self.model_name == "karlo":
                    if self.prior:
                        ...
                        self.logger.info("Applied LoRA to Karlo Prior")
                    if self.decoder:
                        self.decoder = get_peft_model(self.decoder, lora_config_diffusion)
                        self.logger.info("Applied LoRA to Karlo Decoder")
            elif self.unet:
    ...
    def _encode_prompt_sdxl(self, prompt_batch):
        ...
        for tokenizer, text_encoder in zip(tokenizers, text_encoders):
            ...
            pooled_prompt_embeds = encoder_outputs.pooler_output if hasattr(encoder_outputs, 'pooler_output') and encoder_outputs.pooler_output is not None else encoder_outputs.last_hidden_state[:, 0, :]
                ...
                try:
                    ...
                    with autocast(dtype=torch.float16):
                        if self.model_name in ["sdxl", "kandinsky"]:
                            ...
                            latent_dist = vae_output.latent_dist if isinstance(self.vae, AutoencoderKL) else vae_output
                            latents = latent_dist.sample() if isinstance(latent_dist, torch.distributions.Distribution) else latent_dist.latents
                            ...
                            target_values = noise
                            processed_prompts = [summarize_prompt(p, max_tokens=64) for p in prompts]
                            inputs = self.tokenizer(
                                processed_prompts, padding="max_length", max_length=64, truncation=True, return_tensors="pt"
                            ).to(self.device)
                            ...
                            if model_pred.shape[1] != target_values.shape[1]:
                                self.logger.warning(f"Model pred channels ({model_pred.shape[1]}) != target channels ({target_values.shape[1]}). Slicing.")
                                model_pred = model_pred[:, :target_values.shape[1], :, :]
                            loss = torch.nn.functional.mse_loss(model_pred.float(), target_values.float(), reduction="mean")
                    if not torch.isnan(loss):
                        total_loss += loss.item()
                        num_batches += 1
                    del pixel_values, prompts, loss, model_pred, encoder_hidden_states, noisy_input, target_values
                    gc.collect()
    ...
    def save_lora_weights(self, epoch, val_loss, hyperparameters):
        ...
        if save_paths:
            self.logger.info("Saved weights to:")
    ...
    def generate_image(self, prompt_template, topic_description, theory="cbt"):
        ...
        try:
            ...
            if self.model_name == "deepfloyd_if":
                prompt = summarize_prompt(prompt, max_tokens=64)
            if self.unet:
                self.unet.eval()
            if self.text_encoder:
                self.text_encoder.eval()
    ...
    def fine_tune(self, dataset_path, epochs=1, batch_size=1, learning_rate=1e-5, val_split=0.2, gradient_accumulation_steps=8):
        ...
        self.logger.info(f"Dataset: {dataset_path}, Epochs: {epochs}, Batch Size: {batch_size}, LR: {learning_rate}, Val Split: {val_split}, Grad Accum Steps: {gradient_accumulation_steps}")
        try:
            ...
            self.unet, self.text_encoder, optimizer, train_dataloader, val_dataloader = accelerator.prepare(
                self.unet, self.text_encoder, optimizer, train_dataloader, val_dataloader
            )
                        ...
                        with accelerator.accumulate(self.unet):
                            ...
                            with autocast(dtype=torch.float16):
                                if self.model_name in ["sdxl", "kandinsky"]:
                                    ...
                                    self.logger.info(f"Memory before text encoder: {torch.cuda.memory_allocated(self.device)/1e9:.2f} GiB")
                                    encoder_hidden_states = self.text_encoder(inputs.input_ids)[0]
                                    self.logger.info(f"Encoder hidden states shape: {encoder_hidden_states.shape}, memory: {encoder_hidden_states.element_size() * encoder_hidden_states.nelement() / 1e6:.2f} MB")
                                    if torch.isnan(encoder_hidden_states).any() or torch.isinf(encoder_hidden_states).any():
                                        continue
                                    self.logger.info(f"Memory before UNet: {torch.cuda.memory_allocated(self.device)/1e9:.2f} GiB")
                                    model_pred = self.unet(
                                        noisy_input, timestep=timesteps, encoder_hidden_states=encoder_hidden_states
                                    ).sample
                                    if model_pred.shape[1] != target_values.shape[1]:
                            ...
                            del pixel_values, prompts, loss, model_pred, encoder_hidden_states, noisy_input, target_values, latents
                            gc.collect()
                            torch.cuda.empty_cache()
                        global_step += 1
                    except Exception as e:
...
if __name__ == "__main__":
    ...
    model = FinetuneModel(model_name="deepfloyd_if", output_dir="/home/iris/Documents/deep_learning/experiments/custom_finetuned/deepfloyd_if", logger_instance=logger)
    model.load_model()
