...
def log_tensor_stats(tensor, name):
    ...
    if tensor is not None:
        logger.debug(f"{name} stats: shape={tensor.shape}, dtype={tensor.dtype}, "
                     f"min={tensor.min().item():.4f}, max={tensor.max().item():.4f}, "
                     f"mean={tensor.mean().item():.4f}, std={tensor.std().item():.4f}")

def summarize_prompt(prompt, max_tokens=300):
    """Summarize prompt to fit token limit while retaining key themes."""
    words = prompt.split()
...
class SDXLChunkingPipeline(StableDiffusionXLPipeline):
    ...
    def _split_prompt(self, prompt):
        """Split prompt into chunks of chunk_size tokens."""
        words = prompt.split()
        chunks = []
    ...
    def encode_prompt(self, prompt, device, num_images_per_prompt=1, do_classifier_free_guidance=True, negative_prompt=None):
        ...
        prompt_chunks = [self._split_prompt(prompt)] if isinstance(prompt, str) else [self._split_prompt(p) for p in prompt]
        prompt_chunks = [prompt_chunks] if isinstance(prompt, str) else prompt_chunks
        logger.debug(f"Prompt chunks: {[len(chunks) for chunks in prompt_chunks]}")

        negative_chunks = [self._split_prompt(negative_prompt)] if isinstance(negative_prompt, str) else [self._split_prompt(np) for np in negative_prompt]
        negative_chunks = [negative_chunks] if isinstance(negative_prompt, str) else negative_chunks
        ...
        for prompt_idx in range(batch_size):
            ...
            prompt_embeds = torch.cat(prompt_chunk_embeds, dim=1) if prompt_chunk_embeds else torch.zeros(
                (1, self.tokenizer.model_max_length, self.text_encoder.config.hidden_size + self.text_encoder_2.config.hidden_size), dtype=dtype, device=device)
            ...
            pooled = prompt_chunk_pooled[-1] if prompt_chunk_pooled else torch.zeros(
                (1, self.text_encoder_2.config.hidden_size), dtype=dtype, device=device)
            ...
            negative_chunk_embeds, negative_chunk_pooled = [], []
            ...
            negative_embeds = torch.cat(negative_chunk_embeds, dim=1) if negative_chunk_embeds else torch.zeros_like(prompt_embeds)
            ...
            pooled = negative_chunk_pooled[-1] if negative_chunk_pooled else torch.zeros(
                (1, self.text_encoder_2.config.hidden_size), dtype=dtype, device=device)
    ...
    def _get_add_time_ids(self, original_size, crops_coords_top_left, target_size, dtype, text_encoder_projection_dim=None):
        ...
        original_size = original_size or (128, 128)
        ...
        logger.debug(f"Validated inputs: original_size=({original_height}, {original_width}), "
                     f"crops_coords_top_left=({crop_top}, {crop_left}), target_size=({target_height}, {target_width})")
    ...
    def __call__(self, prompt, height=128, width=128, num_inference_steps=15, guidance_scale=9.0, negative_prompt=None, num_images_per_prompt=1, **kwargs):
        ...
        prompt_embeds, added_cond_kwargs = self.encode_prompt(prompt, device, num_images_per_prompt, guidance_scale > 1.0, negative_prompt)
        ...
        latents = self.prepare_latents(batch_size, self.unet.config.in_channels, height, width, dtype, device, None).to(dtype)
        ...
        add_time_ids = self._get_add_time_ids((height, width), (0, 0), (height, width), dtype=dtype).to(device)
        ...
        for t in timesteps:
            ...
            torch.cuda.empty_cache()
            noise_pred = self.unet(latent_model_input.to(device, dtype=dtype), t,
                                  encoder_hidden_states=prompt_embeds.to(device, dtype=dtype),
                                  added_cond_kwargs={k: v.to(device, dtype=dtype) for k, v in added_cond_kwargs.items()}).sample
...
def load_csv_rows_by_ids(csv_path, id_list, chunk_size=50):
    ...
    for chunk in pd.read_csv(csv_path, chunksize=chunk_size):
        ...
        for _, row in selected_rows.iterrows():
            ...
            prompt_parts = [f"{key}: {value}" for key, value in row_dict.items() if key not in ["file"] and pd.notnull(value)]
...
def upscale_image(image_path, output_path, target_size=(1024, 1024)):
    """Upscale a single image to the target size and save it."""
    try:
...
def refine_prompt_with_openai(openai_util, prompt, tokenizer, target_max_tokens=300):
    """Refine prompt using OpenAI API to add visual cues within token limit."""
    system_prompt = (
    ...
    user_prompt = (
        f"Refine the following prompt by adding descriptive visual cues (e.g., a man feeding his dog with 2 children playing in the background) to enhance its visual coherence for image generation, "
    ...
    try:
        ...
        return f"{prompt}, with vibrant and emotional vibe"
...
def generate_images_for_csv_rows(
    ...
    accelerator = Accelerator(cpu=False, mixed_precision="no", device_placement=True)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
