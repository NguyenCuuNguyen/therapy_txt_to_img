...
class KandinskyV22PriorPipelineWithT5(KandinskyV22PriorPipeline):
    ...
    def __init__(self, text_encoder, tokenizer, prior, scheduler, image_encoder, image_processor, logger_instance=None):
        ...
        def custom_forward(
            ...
            hidden_states,
            timestep, # Timestep is usually just an int/tensor, not embed
            encoder_hidden_states=None,
            proj_embedding=None, # Often timestep embeds or image embeds
            attention_mask=None,
            **kwargs,
        ):
            ...
            if attention_mask is not None:
                # Check and potentially adapt attention mask shape if needed
                self_prior.logger.info(f"PriorTransformer.forward (Custom): attention_mask received shape: {attention_mask.shape}, dtype: {attention_mask.dtype}")


            # --- Logic mimicking the specific state from the error log ---
            # NOTE: This concatenation order/logic is likely NON-STANDARD

            # Concatenate proj_embedding (timestep embeds [bs*2, 1, 1280])
            if proj_embedding is not None:
            ...
            # Concatenate encoder_hidden_states (text embeds [bs*2, 77, 1280])
            if encoder_hidden_states is not None:
            ...
            # Add positional embedding with CORRECTED SLICING
            if hasattr(self_prior, 'positional_embedding'):
                 ...
                 if isinstance(pos_embedding_module, torch.nn.Embedding):
                     ...
                     pos_embedding_weights = pos_embedding_module
                      ...
                      pos_embedding_weights = pos_embedding_module # Try direct use

                 if pos_embedding_weights is not None:
                     self_prior.logger.info(f"PriorTransformer.forward (Custom): Full positional_embedding shape: {pos_embedding_weights.shape}") # Should be [?, 81, 2048]
                     ...
                     # --- END FIX ---
                     self_prior.logger.info(f"PriorTransformer.forward (Custom): Sliced positional_embedding shape: {positional_embedding_sliced.shape}, dtype: {positional_embedding_sliced.dtype}") # Should log [?, 79, 2048]
            ...
            # Prepare attention mask (This logic might need adjustment based on actual prior implementation)
            # The passed attention_mask has shape [2, 512] from T5, which is wrong for seq len 79.
            # Creating a dummy mask might be necessary if the standard prior expects one.
            # For now, let's skip using the passed mask as it's incorrect.
            effective_attention_mask = None # Set to None, let transformer blocks handle if possible


            # Pass through transformer blocks
            if hasattr(self_prior, 'transformer_blocks'):
                for block in self_prior.transformer_blocks:
                    hidden_states = block(hidden_states, attention_mask=effective_attention_mask) # Pass None mask
            else:
                 self_prior.logger.error("PriorTransformer object has no attribute 'transformer_blocks'")


            # Final projection - standard prior might project differently
            # This custom forward seems to take the last token? Kandinsky prior usually uses mean or takes specific token.
            # Let's mimic the log's apparent behavior - taking last token's features.
            if hasattr(self_prior, 'out'):
                 # Taking last token's features along the sequence dimension
                 last_token_features = hidden_states[:, -1:, :] # Keep seq dim of size 1
                 predicted_image_embedding = self_prior.out(last_token_features)
                 self_prior.logger.info(f"PriorTransformer.forward (Custom): predicted_image_embedding shape: {predicted_image_embedding.shape}")
            else:
                 self_prior.logger.error("PriorTransformer object has no attribute 'out'")
                 predicted_image_embedding = hidden_states # Fallback?

            return predicted_image_embedding # Should return predicted image embeds
        ...
        # Re-register modules - might not be necessary if modifying self.prior directly
        # self.register_modules(...)

    # _encode_prompt: Use the version from the last prompt that produced the error log
    def _encode_prompt(self, prompt, device, num_images_per_prompt, do_classifier_free_guidance, negative_prompt=None):
        ...
        self.logger.debug(f"Encoding prompt: {prompt[0][:100] if prompt else 'None'}...") # Log first prompt safely

        batch_size = len(prompt) if isinstance(prompt, list) else 1
        if isinstance(prompt, str): prompt = [prompt] # Ensure list
        ...
        attention_mask = text_inputs.attention_mask # Shape [bs, 512]
        ...
        self.logger.info(f"_encode_prompt: last_hidden_state shape: {last_hidden_state.shape}, dtype: {last_hidden_state.dtype}") # Usually float32

        # Pool embeddings
        prompt_embeds = last_hidden_state.mean(dim=1)
        self.logger.info(f"_encode_prompt: After mean pooling, prompt_embeds shape: {prompt_embeds.shape}, dtype: {prompt_embeds.dtype}") # Should log float32

        # --- FIX: Convert dtype BEFORE projection ---
        target_dtype = self.projection_layer.weight.dtype # Get the layer's actual dtype (e.g., float16)
        ...
        self.logger.info(f"_encode_prompt: After dtype conversion, prompt_embeds shape: {prompt_embeds.shape}, dtype: {prompt_embeds.dtype}") # Should log float16 now
        # --- END FIX ---

        # Project embeddings to PriorTransformer input dimension
        prompt_embeds = self.projection_layer(prompt_embeds) # Shape: [bs, 1280] - DTYPES SHOULD MATCH NOW
        # This line is likely redundant now but harmless if target_dtype is float16
        # prompt_embeds = prompt_embeds.to(dtype=torch.float16)
        self.logger.debug(f"Projected prompt embeds shape: {prompt_embeds.shape}, dtype: {prompt_embeds.dtype}")

        # Replicate embeddings to match PriorTransformer's sequence length (77 from config)
        text_sequence_length = 77
        prompt_embeds = prompt_embeds.unsqueeze(1).repeat(1, text_sequence_length, 1) # Shape: [bs, 77, 1280]
        self.logger.info(f"_encode_prompt: After replication, prompt_embeds shape: {prompt_embeds.shape}, dtype: {prompt_embeds.dtype}")

        # text_encoder_hidden_states = prompt_embeds # Keep reference if needed elsewhere
        ...
        if do_classifier_free_guidance:
            ...
            negative_attention_mask = negative_inputs.attention_mask # Shape [bs, 512]
            ...
            negative_embeds_pooled = negative_output.last_hidden_state.mean(dim=1) # This is float32
            ...
            # --- FIX: Convert negative embeds dtype BEFORE projection ---
            # Use the same target_dtype determined above
            ...
            self.logger.info(f"_encode_prompt: After dtype conversion, negative_embeds_pooled shape: {negative_embeds_pooled.shape}, dtype: {negative_embeds_pooled.dtype}") # Should log float16 now
            # --- END FIX ---

            negative_embeds_projected = self.projection_layer(negative_embeds_pooled) # Dtypes match now
            # This line is likely redundant now
            # negative_embeds_projected = negative_embeds_projected.to(dtype=torch.float16)
            negative_embeds = negative_embeds_projected.unsqueeze(1).repeat(1, text_sequence_length, 1) # Shape: [bs, 77, 1280]
            ...
            prompt_embeds = torch.cat([negative_embeds, prompt_embeds], dim=0) # Shape: [2 * bs, 77, 1280]
            # Concatenate original T5 masks for use by the custom forward
            # WARNING: This mask shape [2*bs, 512] does not match sequence length 77 or 79!
        ...
        # Repeat for multiple images per prompt (Not typically done here for prior)
        # prompt_embeds = prompt_embeds.repeat_interleave(num_images_per_prompt, dim=0)
        # text_mask = text_mask.repeat_interleave(num_images_per_prompt, dim=0) # Mask needs repeating too if embeds are

        self.logger.debug(f"Final prompt embeds shape: {prompt_embeds.shape}, dtype: {prompt_embeds.dtype}")
        # Return combined embeds and the MISMATCHED T5 attention mask
        # The custom forward needs to handle this mask issue.
        return prompt_embeds, None, text_mask # Returning None for text_encoder_hidden_states for clarity

    # Custom __call__ method from the last prompt, slightly adapted
    def __call__(
        self,
        prompt,
        negative_prompt=None,
        num_images_per_prompt=1, # Usually 1 for prior
        num_inference_steps=25, # Default steps for prior
        generator=None,
        guidance_scale=4.0, # Typical prior guidance scale
        output_type = "pt", # Request tensor output
        **kwargs # Capture other args like return_dict
    ):
        # Note: This __call__ logic deviates significantly from base class

        # 1. Encode Prompt using overridden _encode_prompt
        ...
        do_classifier_free_guidance = guidance_scale > 1.0 # Standard check
        prompt_embeds, _, text_mask = self._encode_prompt( # Get combined embeds and T5 mask
            prompt=prompt,
            device=device,
            num_images_per_prompt=num_images_per_prompt, # Usually 1
            do_classifier_free_guidance=do_classifier_free_guidance,
            negative_prompt=negative_prompt
        )
        # prompt_embeds shape: [batch_size * (1 if not CFG else 2), 77, 1280]
        # text_mask shape: [batch_size * (1 if not CFG else 2), 512] - Problematic shape

        # 2. Prepare Timesteps
        effective_batch_size = prompt_embeds.shape[0] # Already doubled if CFG

        # --- FIX: Verify scheduler and handle set_timesteps return value ---
        if self.scheduler is None:
            self.logger.error("__call__: self.scheduler is None before calling set_timesteps!")
            raise ValueError("Scheduler object is missing from the pipeline.")
        ...
        scheduler_timesteps_return_value = self.scheduler.set_timesteps(num_inference_steps, device=device) # Use scheduler timesteps

        self.logger.info(f"__call__: Result of set_timesteps call: {scheduler_timesteps_return_value}")

        # Check if the return value is None, if so, try using the internal attribute
        if scheduler_timesteps_return_value is None:
            ...
            scheduler_timesteps = scheduler_timesteps_return_value # Use the returned value if it's valid
            self.logger.info(f"__call__: Using returned scheduler_timesteps. Shape: {scheduler_timesteps.shape}")
        # --- END FIX ---
        # This gives list of [eff_bs, 1280]. Stack later or pass one by one?
        # The loop in the log suggests passing one by one? Let's recalculate inside loop.
        self.logger.info(f"__call__: Using {num_inference_steps} inference steps.")


        # 3. Prepare initial hidden_states (noise) for custom forward
        # Shape needs to match what custom_forward concatenates first.
        # Log shows [bs*2, 1, 1280]. Let's use this.
        ...
        # 4. Denoising Loop (mimicking the log's apparent loop structure)
        # This custom loop likely replaces the scheduler steps logic
        current_embeds = None # Need to track state if it's iterative
        num_loop_steps = num_inference_steps # Match logged steps
        predicted_outputs = [] # Collect outputs if needed

        # --- FIX: Define timestep_proj_dim before the loop ---
        # This dimension is used inside the loop for sinusoidal embeddings
        timestep_proj_dim = self.prior_embed_dim # Use the prior's expected dim (e.g., 1280)
        self.logger.info(f"__call__: Using timestep projection dimension: {timestep_proj_dim}")
        # --- END FIX ---


        # Ensure scheduler_timesteps is iterable before passing to progress_bar
        if scheduler_timesteps is None or not hasattr(scheduler_timesteps, '__iter__'):
             self.logger.error(f"__call__: scheduler_timesteps is not iterable before the main loop! Value: {scheduler_timesteps}")
             raise TypeError("Cannot iterate over scheduler_timesteps in the main denoising loop.")

        self.logger.info(f"__call__: Starting denoising loop with {len(scheduler_timesteps)} timesteps.")

        for i, t in enumerate(self.progress_bar(scheduler_timesteps)): # Use scheduler timesteps
            # Prepare timestep embedding for this step
            ...
            timestep_emb = timestep_emb.unsqueeze(1).to(dtype=dtype) # Shape [eff_bs, 1, 1280]

            # Use init_hidden_states repeatedly? Or update? Log isn't clear.
            # Let's assume init_hidden_states is used each time based on the log.
            loop_hidden_states = init_hidden_states

            # Call the bound custom prior forward method
            # Pass arguments based on the custom_forward definition/log trace
            output = self.prior.forward(
                # self_prior=self.prior, # Pass instance for bound method context By removing the line self_prior=self.prior,, the custom_forward function will receive the self.prior instance exactly once via the implicit self (which maps to self_prior in your definition), resolving the TypeError.
                hidden_states=loop_hidden_states,          # Noise [eff_bs, 1, 1280]
                timestep=t,                                # Pass timestep value
                encoder_hidden_states=prompt_embeds,       # Text embeds [eff_bs, 77, 1280]
                proj_embedding=timestep_emb,           # Timestep embeds [eff_bs, 1, 1280]
                attention_mask=text_mask               # Incorrect T5 mask [eff_bs, 512]
            )
            # Output shape should be [eff_bs, 1, prior_output_dim]? Log shows [bs*2, 1, 1280]
            predicted_outputs.append(output) # Collect outputs

        # 5. Process Outputs
        # The log implies taking the mean, but this doesn't make sense for diffusion prior
        # A prior usually outputs the final predicted embedding directly.
        # Let's assume the *last* output is the relevant one.
        if not predicted_outputs:
             raise ValueError("Inference loop did not produce any outputs.")
        predicted_image_embedding = predicted_outputs[-1] # Take the last step's prediction

        # The output shape seems to be [eff_bs, 1, output_dim]. Squeeze seq dim?
        if predicted_image_embedding.shape[1] == 1:
             predicted_image_embedding = predicted_image_embedding.squeeze(1) # Shape [eff_bs, output_dim]


        self.logger.info(f"__call__ (Custom): final predicted_image_embedding shape: {predicted_image_embedding.shape}")

        # 6. Handle CFG for final output (split neg/pos) - Standard practice
        if do_classifier_free_guidance:
            ...
            # Keep uncond embeds for negative output if needed
        ...
        # 7. Return output matching base class structure
        if output_type == "pt":
            output = image_embeds
        else:
             # Handle other output types if necessary (e.g., PIL Image - N/A for prior)
             raise ValueError(f"Unsupported output_type: {output_type}")


        # Replicate KandinskyV22PriorPipeline.Output format (or use named tuple)
...
# --- FinetuneModel Class ---
# Use the FinetuneModel class definition from the previous response,
# ensuring it calls the modified KandinskyV22PriorPipelineWithT5 pipeline correctly.
# Key parts:
# - load_model: Creates the custom pipeline instance.
# - modify_architecture: Applies LoRA to UNet and the T5 parts (text_encoder + projection_layer).
# - fine_tune: Runs the training loop, calls prior_pipeline, calculates loss on UNet output.
# - validate: Similar to fine_tune but without optimization.
# - save_model_state: Saves LoRA adapters and projection layer.
...
class FinetuneModel:
    def __init__(self, model_name, output_dir, accelerator: Accelerator, logger_instance=None):
        ...
        self.logger = logger_instance or logging.getLogger(__name__)
        self.device = accelerator.device # Use accelerator device
        self.dtype = torch.float16 # Hardcode for now
        self.tokenizer = None
    ...
    def load_model(self):
        ...
        try:
            # 1. Load T5 components (on CPU first, move with pipeline)
            ...
            # 3. Create the custom prior pipeline instance
            ...
            # Set dtype and move to device *after* full init
            ...
            # --- *** FIX: Explicitly move projection_layer if needed *** ---
    ...
    def modify_architecture(self, apply_lora_to_unet=True, apply_lora_to_text_encoder=True):
        ...
        # (Use implementation from previous response - applying LoRA to self.unet
        #  and self.prior_pipeline.text_encoder, making projection_layer trainable manually)
        ...
        if apply_lora_to_text_encoder and hasattr(self.prior_pipeline, 'text_encoder') and hasattr(self.prior_pipeline, 'projection_layer'):
            ...
            try:
                ...
                # Manually make projection layer trainable
    ...
    def validate(self, val_dataloader, dataset_path):
        ...
        # (Use implementation from previous responses - it should work if pipeline call is fixed)
        self.logger.info(f"--- Running Validation Epoch {self.current_epoch} ---")
        if not all([self.unet, self.prior_pipeline, self.vae, self.scheduler]): # Removed text_encoder/tokenizer check as they are inside pipeline
        ...
        with torch.no_grad():
            for step, batch in enumerate(val_dataloader):
                # Ensure batch processing is robust
                try:
                    ...
                    for img_filename, prompt in zip(image_filenames, prompts):
                        try:
                            ...
                            image_np = np.array(image).astype(np.float32) / 255.0 # Normalize 0-1
                    ...
                    pixel_values = torch.stack(pixel_values_list).to(self.accelerator.device) # Move to device
                    prompts = valid_prompts

                    # Generate image embeds using the prior pipeline
                    prior_output = self.prior_pipeline(
                        prompt=prompts,
                        num_inference_steps=25,
                        generator=torch.Generator(device=self.accelerator.device).manual_seed(42+step) # Use different seeds?
                    )
                    image_embeds = prior_output.image_embeds # Base call provides the positive embeds

                    # --- UNet Loss Calculation ---
                    pixel_values_norm = pixel_values.to(dtype=torch.float32) * 2.0 - 1.0 # Normalize -1 to 1 for VAE
                    vae_output = self.vae.encode(pixel_values_norm)
                    latents = vae_output.latents
                    scaling_factor = getattr(self.vae.config, 'scaling_factor', 0.18215)
                    latents = (latents * scaling_factor).to(dtype=self.dtype) # Match UNet dtype
                    ...
                    model_pred = self.unet(
                        ...
                        timestep=timesteps,
                        encoder_hidden_states=image_embeds.to(dtype=self.dtype)
                    ).sample

                    val_loss = torch.nn.functional.mse_loss(model_pred.float(), noise.float(), reduction="mean")

                    if not torch.isnan(val_loss):
                        # Gather loss across processes if distributed
                        gathered_loss = self.accelerator.gather(val_loss.repeat(batch['image'].shape[0] if isinstance(batch['image'], torch.Tensor) else len(batch['image']))) # Ensure gathering matches batch size
    ...
    def save_model_state(self, epoch=None, val_loss=None, hyperparameters=None):
        ...
        # Save Projection Layer (if trained and maybe not saved by PEFT)
        if hasattr(self.prior_pipeline, 'projection_layer'):
    ...
    def fine_tune(self, dataset_path, train_val_splits, epochs=1, batch_size=1, learning_rate=1e-5, gradient_accumulation_steps=1):
        ...
        # (Use implementation from previous response, ensuring optimizer params and accelerator prep are correct)
        ...
        global_best_model_state = None # Not implemented saving best state across folds here
        ...
        for fold_idx, (train_dataset, val_dataset) in enumerate(train_val_splits):
            ...
            self.best_val_loss = float('inf') # Reset best loss for fold
            self.best_epoch = -1

            try:
                train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True) # Added workers/pin_memory
                val_dataloader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)
                image_folder = os.path.join(os.path.dirname(dataset_path), "images") # Assuming structure
                ...
                # Add projection layer params if trainable AND not already included by PEFT model .parameters() call
                if hasattr(self.prior_pipeline, 'projection_layer'):
                    is_proj_trainable = any(p.requires_grad for p in self.prior_pipeline.projection_layer.parameters())
                    if is_proj_trainable:
                         ...
                         if proj_params_to_add:
                              ...
                              self.logger.info("Projection layer params assumed included in text encoder PEFT params.")

                if not params_to_optimize:
                     self.logger.error("Optimizer params list is empty!"); continue # Skip fold

                params_to_optimize = list({id(p): p for p in params_to_optimize}.values()) # Ensure unique params
                ...
                except NameError: # Fallback if bitsandbytes not installed
                ...
                # Prepare text encoder if LoRA applied? Check PEFT docs with Accelerator.
                # If PEFT model params are in optimizer, maybe don't need to prepare model itself.
                # Let's only prepare UNet for now. Projection layer is part of prior_pipeline.
                ...
                # Move non-prepared models to device
                self.prior_pipeline.to(self.accelerator.device)
                self.vae.to(self.accelerator.device)
                self.vae.eval()

                # Hyperparameters for logging/saving
                ...
                for epoch in range(epochs):
                    ...
                    # Set models to train mode as needed
                    self.unet.train()
                    self.prior_pipeline.text_encoder.train()
                    if hasattr(self.prior_pipeline, 'projection_layer') and any(p.requires_grad for p in self.prior_pipeline.projection_layer.parameters()):
                        self.prior_pipeline.projection_layer.train()
                    ...
                    for step, batch in enumerate(train_dataloader):
                        ...
                        # self.logger.debug(f"Step {step+1}/{len(train_dataloader)}")
                        try:
                            # Data loading
                            image_filenames = batch.get('image')
                            prompts = batch.get('prompt')
                            ...
                            pixel_values = torch.stack(pixel_values_list).to(self.accelerator.device) # Move to device
                            prompts = valid_prompts
                            # self.logger.debug(f"Batch prompts: {prompts}")

                            # Accumulate gradients
                            with self.accelerator.accumulate(self.unet): # Accumulate on a model being trained
                                # 1. Prior Pipeline Forward (T5 encoding + Prior Diffusion)
                                # Runs _encode_prompt, then base __call__'s loop -> prior.forward
                                prior_output = self.prior_pipeline(
                                    prompt=prompts,
                                    # Let base __call__ handle default CFG / negative prompts
                                    num_inference_steps=25, # Or other suitable value
                                    generator=torch.Generator(device=self.accelerator.device).manual_seed(42 + global_step) # Seed per step
                                )
                                image_embeds = prior_output.image_embeds.to(dtype=self.dtype) # Ensure correct dtype for UNet
                                # self.logger.debug(f"Prior output image_embeds shape: {image_embeds.shape}")


                                # 2. VAE Encode and Noise Addition
                                with torch.no_grad():
                                ...
                                # self.logger.debug(f"Latents shape: {latents.shape}")
                                ...
                                timesteps = torch.randint(0, self.scheduler.config.num_train_timesteps, (latents.shape[0],), device=latents.device).long()
                                noisy_latents = self.scheduler.add_noise(latents, noise, timesteps)
                                # self.logger.debug(f"Noisy latents shape: {noisy_latents.shape}, Timesteps: {timesteps}")
                                ...
                                # self.logger.debug(f"UNet model_pred shape: {model_pred.shape}")
                                ...
                                if self.accelerator.sync_gradients:
                                    if params_to_optimize:
                                         ...
                                         # self.logger.debug(f"Grad norm: {grad_norm.item()}")
                                    self.optimizer.step()
                                    self.optimizer.zero_grad()

                                # Accumulate loss for logging epoch avg
                                train_loss_epoch += self.accelerator.gather(loss).mean().item() * gradient_accumulation_steps # Account for accumulation?
                                num_train_batches_epoch += 1


                                # Logging rank 0 loss periodically
                    ...
                    if val_dataloader:
                         # Simplified: Run validation only on main process for simplicity
                         if self.accelerator.is_main_process:
                              avg_val_loss = self.validate(val_dataloader, dataset_path)
                         # Need barrier to ensure main process waits if others finish early
                         self.accelerator.wait_for_everyone()
                    else:
                         if self.accelerator.is_main_process: self.logger.info("Skipping validation (no validation set).")
                         # If no validation, maybe save checkpoint every epoch?
                         # avg_val_loss = avg_train_loss_epoch # Use train loss if no val


                    # Check for best model and save on main process
                    if self.accelerator.is_main_process:
                        current_loss_for_comparison = avg_val_loss if val_dataloader else avg_train_loss_epoch # Choose metric
                    ...
                    # Barrier to ensure saving is done before next epoch or fold end
                    self.accelerator.wait_for_everyone()
                ...
                # Log best loss for the fold (from main process perspective)
                if self.accelerator.is_main_process:
                ...
                fold_val_losses.append(float('inf')) # Record failure

            self.accelerator.wait_for_everyone() # Sync before next fold


        # --- End of K-Fold Loop ---
        # Final reporting (on main process)
        if self.accelerator.is_main_process:
            ...
            # Note: Selecting the 'best' overall model state requires more logic (e.g., tracking best state across folds)

        # Return average loss (might need more sophisticated aggregation in distributed setting)
        return avg_val_loss, None, None, None # Placeholder for best state return
...
if __name__ == "__main__":
    ...
    CONFIG_PATH = "config.yaml" # CHANGE THIS PATH
    ...
    accelerator = Accelerator(
        ...
        log_with="tensorboard", # Example logging integration
        project_dir=os.path.join(output_dir, "logs")
    )
    logger.info(f"Accelerator initialized on device: {accelerator.device}")
    # Set logger level based on main process
    logging.getLogger().setLevel(logging.DEBUG if accelerator.is_main_process else logging.ERROR)


    # 3. Load Dataset Splits (Example)
    # This needs implementation based on your dataset format/structure
    try:
        # Assuming load_dataset returns a list of (train_idx, val_idx) tuples or similar
        # And CocoFinetuneDataset can be initialized with indices/filenames
        logger.info(f"Loading dataset from: {dataset_path} with {num_folds} folds")
        # Dummy implementation - replace with actual dataset loading
        # all_data = [{"image": f"img_{i}.jpg", "prompt": f"prompt {i}"} for i in range(100)]
        # kf = KFold(n_splits=num_folds, shuffle=True, random_state=42)
        # train_val_splits_data = []
        # for train_idx, val_idx in kf.split(all_data):
        #    train_data = [all_data[i] for i in train_idx]
        #    val_data = [all_data[i] for i in val_idx]
        #    train_dataset = CocoFinetuneDataset(train_data) # Pass data subset
        #    val_dataset = CocoFinetuneDataset(val_data)     # Pass data subset
        #    train_val_splits_data.append((train_dataset, val_dataset))
        # Replace above dummy logic with your actual load_dataset call
    ...
    # 4. Initialize and Run Finetuning
    try:
        ...
        avg_loss, _, _, _ = finetuner.fine_tune(
            dataset_path=dataset_path, # Pass base path for image folder resolution
        ...
        accelerator.set_trace() # Optional: Trigger breakpoint on error in distributed setting
