...
def load_pipeline_with_lora(model_name, base_model_ids, lora_checkpoint_dir, device, target_dtype):
    ...
    try:
        if model_name == "sdxl_base_refiner":
            ...
            refiner_pool_projection_layer_files = [f for f in os.listdir(lora_checkpoint_dir) if f.endswith("_refiner_pool_projection_layer.pth")]
            ...
            if pool_projection_layer_files:
                ...
                logger.info(f"Loading base pool projection layer from {pool_projection_layer_path}")
                ...
                logger.info("Loaded base pool projection layer.")
            else:
                logger.info("No base pool projection layer file found; using default initialization.")
                pool_projection_layer = torch.nn.Linear(768, 1280).to(device, dtype=target_dtype)

            if refiner_pool_projection_layer_files:
            ...
            pipeline = StableDiffusionXLPipelineWithT5(
                ...
                refiner_pool_projection_layer=refiner_pool_projection_layer,
