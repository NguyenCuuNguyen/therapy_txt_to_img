...
log_file = os.path.join(log_dir, "sdxl_chunks_fullscaled_refiner.log")
...
def log_tensor_stats(tensor, name):
    ...
    if tensor is not None:
        logger.debug(f"{name} stats: shape={tensor.shape}, dtype={tensor.dtype}, device={tensor.device}, min={tensor.min().item():.4f}, max={tensor.max().item():.4f}, mean={tensor.mean().item():.4f}, std={tensor.std().item():.4f}")
...
class SDXLChunkingPipeline(StableDiffusionXLPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Calculate steps for base pipeline
        ...
        del prompt_embeds, add_time_ids
        cleanup_memory()
...
def generate_images_for_csv_rows(
    ...
    yaml_path,
    id_list_path,
    output_dir="/home/iris/Documents/deep_learning/generated_images/sdxl_chunks_fullscaled_refiner",
    chunk_size=1,
):
    ...
    refiner_pipeline.scheduler = DPMSolverMultistepScheduler.from_pretrained(
        "stabilityai/stable-diffusion-xl-refiner-1.0",
        subfolder="scheduler",
        use_safetensors=True,
    )
    ...
    custom_pipeline.unet.to(device, dtype=dtype)
    custom_pipeline.vae.to(device, dtype=dtype)
    custom_pipeline.text_encoder.to(device, dtype=dtype)
    custom_pipeline.text_encoder_2.to(device, dtype=dtype)
    log_memory_usage()

    # Move refiner components to CPU initially, checking for None
    refiner_pipeline.unet.to("cpu")
    refiner_pipeline.vae.to("cpu")
    if refiner_pipeline.text_encoder is not None:
        refiner_pipeline.text_encoder.to("cpu")
    if refiner_pipeline.text_encoder_2 is not None:
        refiner_pipeline.text_encoder_2.to("cpu")
    cleanup_memory()
